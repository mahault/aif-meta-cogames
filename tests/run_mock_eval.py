...
class MockAgent:
    ...
    def __init__(self, agent_id: int):
        ...
        # Pre-seed station knowledge
        for pos, stype in STATIONS.items():
            self.spatial_memory.stations[pos] = stype
...
def step_env(agent: MockAgent, direction: str) -> dict:
    ...
    if new_pos in STATIONS and new_pos != pos:
        ...
        if station == "extractor" and agent.inventory == "empty":
            agent.inventory = "carbon"
            info["interaction"] = "mine"
            agent.just_deposited = False
        elif station == "hub":
            if agent.inventory == "carbon":
                agent.inventory = "empty"
                info["interaction"] = "deposit"
                agent.just_deposited = True
            elif agent.inventory == "empty" and not agent.just_deposited:
                # Only withdraw hearts if not immediately after deposit.
                # In real CogsGuard, withdrawal is deliberate, not automatic.
                agent.inventory = "hearts"
                info["interaction"] = "withdraw_hearts"
                agent.just_deposited = False
        elif station == "craft" and agent.inventory == "hearts":
            agent.inventory = "gear"
            info["interaction"] = "craft_gear"
            agent.just_deposited = False
        elif station == "junction" and agent.inventory == "gear":
            agent.inventory = "empty"
...
def discretize_obs(agent: MockAgent) -> list:
    ...
    # Inventory
    if agent.inventory in ("carbon", "hearts"):
        obs_inv = ObsInventory.HAS_RESOURCE
    elif agent.inventory == "gear":
...
def run_eval(n_agents: int = 2, max_steps: int = 500, verbose: bool = True):
    ...
    print(f"\n{'='*60}")
    print(f"  AIF v9.3 Mock Evaluation — {n_agents} agents, {max_steps} steps")
    print(f"  Grid: 8x8 | Stations: extractor(0,0) hub(2,2) craft(4,4) junction(2,6)")
    print(f"  Agent 0: miner (even) | Agent 1: aligner (odd)")
    print(f"{'='*60}\n")
    ...
    for step in range(max_steps):
        for agent_id in range(n_agents):
            ...
            # 4. Resolve nav target
            target = resolve_nav_target(agent, task_policy)
            ...
            # 9. Detailed nav diagnostics (around stall region)
            ...
            if verbose and (85 <= step <= 105 or (step % 50 == 0 and step > 0)):
                print(
                    ...
                    f"moved={info['moved']} inv={agent.inventory} "
        ...
        if verbose and (step + 1) % 100 == 0:
            ...
            for i in range(n_agents):
                ...
                print(
                    ...
                    f"inv={agents[i].inventory} option={option} task={tp}"
                )
            print()
    ...
    for i in range(n_agents):
        ...
        print(f"  Agent {i} ({role}): mine={m['mine']} deposit={m['deposit']} "
              ...
              f"capture={m['capture']} noops={m['noops']}")
